...
export const Login = (props) => {
  ...
  const onSubmit = async (values) => {
    ...
    try {
      setIsLoading(true);
      const response = await axios({ method: "post", url: LOGIN_ROUTE, data: values });
      console.log(response);
      if ([400, 401, 404].includes(response.status)) {
        console.log("from bad response");
        dispatch({ type: "SET_ERROR_MESSAGE", payload: response.data });
      }
  ...
  return (
    <>
      <div className="container ">
        ...
        <div
          className={`${isLoading ? "display--none" : "flex"} flex--column gap--md align-items--c`}
        ...
        >
          ...
          <Formik initialValues={initialValues} validationSchema={loginValidationSchema} onSubmit={onSubmit}>
            <Form className="mt-1 w--70% flex flex--column align-items--c justify-content--c gap--sm">
              <div className={`text--danger p--xs ${!errorMessage && "display--none"} `}>{errorMessage}</div>
              {/* EMAIL */}
              <div className="form-group">
                <Field type="text" name="email" className="form-control border-radius--sm p--xs" placeholder="Email" />
              ...
              <div className="form-group">
                ...
                />
                <ErrorMessage name="password" component="p" className="text--danger p--xxs " />
              </div>
              <Btn type="submit" variant="primary" className="border-radius--sm w--100%">
                Login
              </Btn>
            </Form>
          </Formik>
